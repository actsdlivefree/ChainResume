...
class FhevmDecryptionSignatureStorageKey {
  ...
  constructor(instance: FhevmInstance, contractAddresses: string[], userAddress: string, publicKey?: string) {
    ...
    const hash = ethers.TypedDataEncoder.hash(
      emptyEIP712.domain,
      { UserDecryptRequestVerification: (emptyEIP712 as EIP712Type).types.UserDecryptRequestVerification },
      emptyEIP712.message
    );
  ...
  static async new(
    ...
    try {
      ...
      const signature = await signer.signTypedData(
        (eip712 as EIP712Type).domain,
        { UserDecryptRequestVerification: (eip712 as EIP712Type).types.UserDecryptRequestVerification },
        (eip712 as EIP712Type).message
      );
...
export type FhevmDecryptionSignatureType = ReturnType<typeof FhevmDecryptionSignature.fromJSON>;

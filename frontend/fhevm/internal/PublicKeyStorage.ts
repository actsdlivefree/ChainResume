...
export async function publicKeyStorageGet(
...
): Promise<{ publicKey?: FhevmInstanceConfig["publicKey"]; publicParams: FhevmInstanceConfig["publicParams"] }> {
  ...
  const publicKey =
    storedPublicKey?.value?.publicKeyId && storedPublicKey?.value?.publicKey
      ? {
